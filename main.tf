...
resource "azurerm_resource_group" "docker-deploy-test-rg" {
...
resource "azurerm_service_plan" "docker-deploy-test-service-plan" {
  ...
  resource_group_name = azurerm_resource_group.docker-deploy-test-rg.name
  location            = azurerm_resource_group.docker-deploy-test-rg.location
  os_type             = "Linux"
  sku_name            = "B1"
}
